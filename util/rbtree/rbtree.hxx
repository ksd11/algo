...
// 左旋转
void RBTree::RotateLeft(RBTreeNode* curNode) {
  RBTreeNode* rightNode = curNode->right; // 右孩子
  curNode->right = rightNode->left;

  if (rightNode->left != nil) {
    rightNode->left->parent = curNode;
  }
  ...
  if (curNode->parent == nil) {
    ...
    curNode->parent->left = rightNode;
  } else {
    curNode->parent->right = rightNode;
  }
...
// 右旋转
void RBTree::RotateRight(RBTreeNode* curNode) {
  RBTreeNode* leftNode = curNode->left; // 未来的父节点
  curNode->left = leftNode->right;

  if (leftNode->right != nil) {
    leftNode->right->parent = curNode;
  }
  ...
  if (curNode->parent == nil) {
    ...
    curNode->parent->left = leftNode;
  } else {
    curNode->parent->right = leftNode;
  }
...
bool RBTree::Insert(const int nData) {
  ...
  RBTreeNode* pPNewNode = nil;  // 记录插入位置
...
RBTreeNode* RBTree::DeleteNode(RBTreeNode* pNode) {
  ...
  if (pDeleteNode->color == BLACK) DeleteFixup(pCDeleteNode);
...
void RBTree::DeleteFixup(RBTreeNode* pNode) {
  while (pNode != root && pNode->color == BLACK) {
    if (pNode == pNode->parent->left) {
      ...
      if (pBNode->left->color == BLACK &&
          pBNode->right->color == BLACK)  // case 2
      {
        pBNode->color = RED;
        pNode = pNode->parent;
...
void RBTree::PrintTree(RBTreeNode* pNode) const {
  ...
  if (pNode == root) {
    std::cout << "[" << ++n << "]nData = " << pNode->nData
              << ",nParentData= 0 ,";

    if (pNode->left) std::cout << "nLeftData= " << pNode->left->nData << " ,";
    if (pNode->right)
    ...
    std::cout << "[" << ++n << "]nData = " << pNode->nData
              << ",nParentData= " << pNode->parent->nData << " ,";
